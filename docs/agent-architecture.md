...
flowchart TD
  ...
  Atom --> Index[改訂の差分と新規AtomをローカルE5で索引化]
...
`memory_focus` は共通runtimeの道具で、モデルが次に調べたい文脈と短い明示的な検討状態を指定する。次のモデル呼び出し前に、それらと直近の観測をベクトル化し、記憶と関係を選び直す。前回自動挿入した記憶を検索入力へ再帰的に混ぜない。
