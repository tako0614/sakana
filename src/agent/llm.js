...
export function runAgent(options) {
  return executeAgent({ ...options, modelIdentity: `openrouter:${agentConfig.model}`, request: ({ messages, tools, effort, deadlineAt, guildId, runId, final, beforeSend, consumeRequest }) => requestModel({
    model: agentConfig.model, messages, tools, guildId, runId, beforeSend, consumeRequest, role: 'chat', deadlineAt: final ? Infinity : deadlineAt,
