...
function buildBody({ messages, tools, dropThinking }) {
  const body = {
  ...
  if (!dropThinking) {
    ...
    if (agentConfig.reasoningEffort) {
      body.reasoning_effort = agentConfig.reasoningEffort;
    }
  }
...
async function requestOnce({ messages, tools, dropThinking, signal }) {
  const response = await fetch(`${agentConfig.baseUrl}/chat/completions`, {
    ...
    body: JSON.stringify(buildBody({ messages, tools, dropThinking })),
    signal
  });
...
async function callModel({ messages, tools, signal }) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      return await requestOnce({ messages, tools, dropThinking, signal });
    } catch (error) {
 ...
 */
export async function runAgent({ system, userContent, toolset, onToolCall, signal }) {
  const messages = [
  ...
  for (; rounds < agentConfig.maxRounds; rounds += 1) {
    ...
    const data = await callModel({
      messages,
      tools: isLastRound ? null : toolset.definitions,
      signal
    });
    ...
    if (data.usage) {
      ...
      usage.prompt_cache_hit_tokens += data.usage.prompt_cache_hit_tokens ?? 0;
    }
    ...
    if (toolCalls.length === 0) {
      return { text: message.content ?? '', rounds: rounds + 1, usage, used };
    }
