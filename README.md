...
AGENT_REQUEST_TOKEN_LIMIT=1000000   # 1リクエストの予算 (暴走ガード)
AGENT_USER_TOKEN_LIMIT=5000000      # 1人あたり
AGENT_USER_WINDOW_MS=3600000        # の 1 時間で
AGENT_GLOBAL_TOKEN_LIMIT=50000000   # 全体
...
**縛るのはトークンだけ**です。往復回数や文字数では縛りません — 軽い一言と全期間の調査が同じ「1回」として数えられて、実際の費用とまるで合わないからです (1回の実行で 1.5万〜10万トークン使います)。
...
**どの値も「暴走と荒らしを止める壁」で、普通に使って引っかかってはいけない水準に置いてあります。** 重い調査の実測が最大10万トークンなので、1リクエストの予算はその10倍 (100万)、1人あたりは時500万 (重い調査を50回)、全体は日5000万です。**管理者 (`ARCHIVE_ADMIN_USERS` / Administrator / ManageGuild) は判定そのものを通りません。**

1リクエストの予算を使い切ったときは「打ち切りました」と言って終わるのではなく、**ツールを外してその場の材料で答えを書かせます**。往復の上限は `HARD_ROUND_CAP = 100` がコードに埋まっていますが、これは無限ループ専用の最後の壁で、つまみとしては出していません (予算で止まるのが普通の経路)。
