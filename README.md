...
読みにくさは実際の回答を見て潰しました。観測した失敗と、それに対応するルール:

| 出た症状 | 入れたルール |
| --- | --- |
| 改行ゼロで一段落に全部詰まる (箇条書き禁止を守ろうとして一枚岩になる) | 話題が変わるところで改行する |
| 1文目で答えているのに末尾で「つまり結論としては〜ということになる」と言い直す | 結論を2回書かない。その言い回しを名指しで禁止 |
| 「AがBって言ってて、貼られた投稿はCだから、Dだと思う」と推論の筋道を見せる | 推論の過程を書かない。C だけ書く |
| 聞かれたことに答えていない傍証で参照3件の枠を埋める | 引用は効くものだけ。枠を埋めない |
| 「〜で、」「〜だから、」で繋ぎ続け、引用リンクが文の途中に何度も割り込む | 1文を短く切る |

「600字くらい」という長さの目標も**消しました**。目標を持たせると、足りないぶんを言葉数で埋めます。
