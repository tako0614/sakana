...
# **bot が意味のある形で出せない発言**。本文がこれ「だけ」の行を損失から外す。
#
# 実測 (564,826 発言):
#   [添付] だけ   16,035 (2.8%)
#   URL だけ      16,907 (3.0%)
#   @名前 だけ     3,808 (0.7%)
#
# どれも文脈としては意味がある (誰かが画像を貼る / リンクを投げる / 呼びかける) が、
# **返答として出されると無意味**。bot は画像もリンクも実際には出せない。
#
# evex-1 では空の発言が `<file>` 1 トークンになっていて、コーパス内 3.6% が
# 返答の 38% を占めた (10倍に増幅)。1 トークンだと発言の先頭で確率が1点に集中する。
# `https://github.com` は複数トークンなので増幅は小さいが、3.0% は放置するほど
# 小さくない。
#
# 2文字以下の発言 (6.4% / 「草」「うん」「w」) は**外さない**。あれは本物の発言で、
# このサーバーの文体そのもの。短すぎる返答は bot 側で引き直す。
UNUSABLE_BODY = re.compile(
    r"^[^:\n]{1,12}: ("
    r"\[(?:画像|動画|音声|ファイル|添付|圧縮ファイル|埋め込み|スタンプ)\]"
    r"|https?://\S+"
    r"|@[^\s]+"
    r")$",
    re.MULTILINE,
...
def attachment_spans(text):
    """返答として使えない発言の**本文の**文字範囲。ラベル (`たこ: `) は含めない。"""
