...
# 外部データに露骨な表現の除去を掛けた世代か。**evex-4 / 4.1 / 5 / 5.1 は掛けてある**
# ので、あれらのカードを作り直すときは付ける。以降の世代は廃止した (下に理由)
parser.add_argument("--explicit-filter", action="store_true",
                    help="外部データに露骨表現の除去を掛けた世代のときだけ付ける")
...
if args.explicit_filter:
    W("- 有害表現の除去は**外部データにだけ**機械的に掛けています "
      "(12 語の正規表現)。**このサーバー自身のログには掛けていません。**"
      "推論側の安全機構もありません")
else:
    W("- **有害表現のフィルタは掛けていません。**学習データにも推論にもありません。")
    W("  以前は外部データにだけ 12 語の正規表現を掛けていましたが、"
      "実測すると外部は 0.09% を除去して残り 0%、一方で**このサーバー自身のログは"
      "フィルタ無しで 0.59% が残る**という逆立ちした状態でした。学習の重心は"
      "サーバー側 (計算量の 80%) なので実質的に何も守れておらず、"
      "「機械的に除去している」という説明だけが誤解を招くため廃止しました")
W("")
