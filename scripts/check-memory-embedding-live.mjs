...
try {
  ...
  const source = await memory.write('ユーザーの追放は管理者が手動で実行する。AIは処分を提案する。');
  const group = await memory.write('コミュニティの権限と役割');
  const relation = await memory.write({text:'処分についての役割分担',links:{group:group.ref,member:source.ref}});
  await memory.write('水槽の魚に餌を与える。');
